...
const Search: React.FC<IProps> = (props) => {
  ...
  useEffect(() => {
    if (customerId == "" || tags == "" || customer == "") setShow(false);
    else setShow(true);
  ...
  const prop: UploadProps = {
    ...
    name: "file",
    action: `http://134.175.81.248:5000/upload/report`,
    headers: {
  ...
  const onClick = () => {
    const params = [];
    if (customerId.trim() == "") params.push("身份证号");
    if (tags.trim() == "") params.push("检查内容");
    if (customer.trim() == "") params.push("患者姓名");
    if (params.length > 0)
      messageApi.open({
        type: "error",
